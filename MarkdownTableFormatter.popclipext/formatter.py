...
def md_table_format(raw_table, margin=1, padding=0, default_justify=Justify.LEFT):
...
if __name__ == "__main__":
    if os.environ['POPCLIP_TEXT']:
        input_text = final_text = os.environ['POPCLIP_TEXT']
        positions = find_all(input_text)
        for start, end in positions:
            prev_table = input_text[start:end]
            new_table = md_table_format(prev_table)
            final_text = final_text.replace(prev_table, new_table)
